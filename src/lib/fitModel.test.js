...
/** The Supra MK5 body, in metres (see data/carStage.js). */
const SUPRA = { length: 4.38, width: 1.87, height: 1.29 };
...
describe('fitToLength', () => {
  it('scales a unit box up to the target length', () => {
    ...
    const fit = fitToLength(box, SUPRA.length);

    expect(fit.scale).toBeCloseTo(4.38);
    expect(fit.size.length).toBeCloseTo(4.38);
    expect(fit.size.width).toBeCloseTo(4.38);
    expect(fit.size.height).toBeCloseTo(4.38);
  });

  it('leaves a box already at the target length untouched in scale', () => {
    const box = {
      min: { x: 10, y: 5, z: -3 },
      max: { x: 10 + SUPRA.length, y: 5 + SUPRA.height, z: -3 + SUPRA.width },
    };
    const fit = fitToLength(box, SUPRA.length);

    expect(fit.scale).toBeCloseTo(1);
    expect(fit.size.width).toBeCloseTo(SUPRA.width);
    expect(fit.size.height).toBeCloseTo(SUPRA.height);
  });

  it('centres the box on the origin in x and z and rests it on the ground', () => {
    const box = { min: { x: 100, y: 7, z: -40 }, max: { x: 140, y: 17, z: -20 } };
    const fitted = apply(fitToLength(box, SUPRA.length), box);
  ...
  it('works for a box that straddles the origin below the ground', () => {
    ...
    expect((fitted.min.x + fitted.max.x) / 2).toBeCloseTo(0);
    expect(fitted.min.y).toBeCloseTo(0);
    expect(fitted.max.x - fitted.min.x).toBeCloseTo(SUPRA.length);
  });
  ...
  it('rejects a box with no length and a target that is not positive', () => {
    ...
    expect(() => fitToLength(flat, SUPRA.length)).toThrow(/length/i);
